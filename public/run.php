...
// Taruh file ini di folder: Fundata/public/run.php
// Akses: https://dev.fundata.id/run.php?key=KODE_RAHASIA&cmd=COMMAND

$SECRET_KEY = 'fundata_secret_2026'; // GANTI dengan kode rahasia kamu sendiri!
...
if (empty($command)) {
    ...
    echo "<html><head><title>Fundata Runner</title>";
    echo "<style>
...
if ($command === 'test-email') {
    ...
    try {
        ...
        \Illuminate\Support\Facades\Mail::raw('Ini adalah email pengetesan dari Fundata diagnostic tool.', function ($message) {
            $message->to(config('mail.from.address')) // kirim ke diri sendiri
                    ->subject('Test Koneksi SMTP Fundata');
        });
