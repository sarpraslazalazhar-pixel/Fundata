...
export default function Dashboard({ recentTickets = [], stats }: { recentTickets?: any[], stats?: any }) {
 ...
 const QUICK_ACTIONS = [
 { label: 'Input Data', desc: 'Buat entri data baru', icon: PlusCircle, href: '/data/buat', color: 'text-primary bg-primary/10' },
 { label: 'Data Saya', desc: 'Lihat semua data Kamu', icon: History, href: '/data/riwayat', color: 'text-emerald-600 bg-emerald-50 ' },
 ];
 ...
 <div className="space-y-4">
 <div className="flex items-center justify-between">
 <h2 className="text-lg font-semibold">Data Terbaru Kamu</h2>
 <Link href="/data/riwayat" className="text-sm font-medium text-primary hover:underline">Lihat Semua</Link>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
 {recentTickets.map((tiket) => (
 <Link key={tiket.id} href={`/data/${tiket.id}`}>
 ...
 <h3 className="text-base font-semibold text-foreground">Belum ada data</h3>
 <p className="text-sm text-muted-foreground mt-1">
 Input data pertama Kamu.
 </p>
 </div>
 <Link href="/data/buat">
