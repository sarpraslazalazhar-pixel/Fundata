...
const STEPS = [
  { label: 'Kategori Data', description: 'Kategori dan jenis data' },
  { label: 'Isi Form', description: 'Formulir pendataan' },
  { label: 'Lampiran', description: 'Unggah dokumen pendukung' },
  { label: 'Tinjau & Simpan', description: 'Periksa dan simpan data' },
];
...
export default function DataWizard({ unitList, campaigns = [], paymentMethods = [], akads = [] }: { unitList: any[], campaigns?: any[], paymentMethods?: any[], akads?: any[] }) {
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 0) {
      if (!data.unit_id) { setError('unit_id', 'Kategori Data wajib dipilih'); isValid = false; }
  ...
  return (
    <UserLayout title="Input Data Baru">
      <div className="max-w-4xl mx-auto py-8 px-4">
        <Head title="Input Data Baru" />
        <h1 className="text-2xl font-bold mb-6">Input Data Baru</h1>
                ...
                >
                  {activeStep === 0 && (
                    <div className="space-y-6">
                      <div>
                        <Label className="mb-3 block text-base font-semibold">Kategori Data <span className="text-red-500">*</span></Label>
                        <RadioCardGrid
                          options={unitList}
                          value={data.unit_id}
                          ...
                          labelKey="nama_unit"
                          showIcon={true}
                          groupId="kategori-data-grid"
                        />
                        {errors.unit_id && <p className="text-red-500 text-sm mt-1">{errors.unit_id}</p>}
                      </div>
                      <div>
                        <Label className="mb-3 block text-base font-semibold">Jenis Data <span className="text-red-500">*</span></Label>
                  ...
                  {activeStep === 1 && (
                    <div>
                      {fieldsLoading ? (
                        ...
                        <p className="text-sm text-slate-500">Silakan pilih jenis data terlebih dahulu.</p>
                      ) : nonUploadFields.length === 0 ? (
                        <p className="text-sm text-slate-500">Tidak ada field form untuk data ini.</p>
                      ) : (
                        <DynamicField
                  ...
                  {activeStep === 2 && (
                    <div>
                      {fieldsLoading ? (
                        ...
                        <p className="text-sm text-slate-500 mt-2">Tidak ada lampiran yang diperlukan untuk data ini.</p>
                      ) : (
                        <div className="space-y-6">
                          {uploadFields
                            .filter(field => {
                  ...
                  {activeStep === 3 && (
                    <div className="space-y-4">
                      <ReviewSection title="Kategori & Jenis Data" icon={<CheckCircle2 className="w-4 h-4 text-green-500" />}>
                        <ReviewRow label="Kategori Data" value={getSelectedName(unitList, data.unit_id)} />
                        <ReviewRow label="Jenis Data" value={getSelectedName(subUnits.options, data.sub_unit_id)} />
                        {data.campaign_id && (
                          <ReviewRow label="Campaign" value={getSelectedName(campaigns, data.campaign_id)} />
                        )}
                      </ReviewSection>
                      ...
                      <p className="text-xs text-slate-400 flex items-center gap-1">
                        ...
                        Pastikan semua entri sudah benar sebelum menyimpan data.
                      </p>
                    </div>
                  )}
                </motion.div>
                ...
                {activeStep < STEPS.length - 1 ? (
                  ...
                  <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                    <Button type="submit" disabled={processing || !canSubmit}>
                      {processing ? "Menyimpan..." : !canSubmit ? "Mohon Tunggu..." : "Simpan Data"}
                    </Button>
                  </motion.div>
                )}
              </div>
