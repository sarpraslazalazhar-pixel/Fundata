...
export default function NotificationsIndex({ notifications, filters }: Props) {
  ...
  const handleAction = async (action: 'read' | 'snooze' | 'done', id: string, extra?: any) => {
    try {
      if (action === 'read') {
        ...
        await axios.patch(route('notifications.snooze', { id }), { snooze_minutes: extra });
      } else if (action === 'done') {
        await axios.patch(route('notifications.done', { id }));
      }
      router.reload({ only: ['notifications'] });
    } catch (error) {
      console.error('Gagal memproses:', error);
  ...
  return (
    <UserLayout title="Semua Notifikasi">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Pusat notifikasi dan pengingat Anda
          </p>
        </div>
      </div>
          ...
          {notifications.data.length === 0 ? (
            ...
            <div className="divide-y">
              {notifications.data.map((notification) => (
                <div 
                  key={notification.id} 
                  className={`p-4 flex flex-col sm:flex-row gap-4 hover:bg-muted/50 transition-colors ${!notification.read_at ? 'bg-blue-50/50 ' : ''}`}
                >
                  <div className="flex-1 min-w-0 flex items-start gap-3">
                    ...
                    <div className="flex-1 cursor-pointer" onClick={() => {
                      ...
                      if (notification.data.aksi_url || notification.data.url) window.location.href = notification.data.aksi_url || notification.data.url;
                    }}>
